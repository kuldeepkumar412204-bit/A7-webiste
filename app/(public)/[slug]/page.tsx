...
import DelhiBazar, {
  metadata as delhiBazarMetadata,
} from "./components/DelhiBazar";
import ShriGanesh, {
  metadata as shriGaneshMetadata,
} from "./components/ShriGanesh";
import Agra, { metadata as agraMetadata } from "./components/Agra";
import Faridabad, {
  metadata as faridabadMetadata,
} from "./components/Faridabad";
import Alwar, { metadata as alwarMetadata } from "./components/Alwar";
import Gahaziabad, {
  metadata as ghaziabadMetadata,
} from "./components/Ghaziabad";
import Dwarka, { metadata as dwarkaMetadata } from "./components/Dwarka";
import Gali, { metadata as galiMetadata } from "./components/Gali";
import DelhiMatka, {
  metadata as delhiMatkaMetadata,
} from "./components/DelhiMatka";
...
export async function generateMetadata({
  ...
  if (market?.seo) {
    const title =
      market.seo.metaTitle || `${slugKey.replace(/-/g, " ")} | A7 Satta King`;

    const description =
      market.seo.metaDescription ||
      `Check ${slugKey.replace(
        /-/g,
        " ",
